...
def crypt_major():
    ...
    return ''.join(map(chr, _crypt.sample(range(256), 256)))

def crypt_minor():
    'Create a new Minor Key.'
    sample = _crypt.sample(list(range(4)) * 64, 256)
    array = []
...
def named_major(name):
    ...
    _namer.seed(name)
    return ''.join(map(chr, _namer.sample(range(256), 256)))

def named_minor(name):
    'Create a named Minor Key.'
    _namer.seed(name)
    sample = _namer.sample(list(range(4)) * 64, 256)
    array = []
    for index in range(64):
        bits_12 = sample[index * 4] << 6
        bits_34 = sample[index * 4 + 1] << 4
...
class Socket_Crypt:
    ...
    def sendall(self, string, flags=0):
        ...
        string = _encode(string, self.__em1, self.__em2)
        return self.__socket.sendall(string.encode(), flags)
