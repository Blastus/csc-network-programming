...
def encode(string, divide=1024):
    ...
    def encode(s, k, b):
        ...
        while i:
            ...
            i /= 255
        return s
    key = ''.join(byte for byte in map(chr, range(256)) if byte in string)
    divide = divide * 256 / len(key)
    base = len(key) + 1
    return '\0'.join(encode(string[index:index+divide], key, base) for index in range(0, len(string), divide)), key
